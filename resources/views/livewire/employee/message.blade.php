    ...
    <div class="bg-[#0f1117] text-[#e6edf3] h-[calc(100vh-48px)] flex overflow-hidden rounded-2xl border border-[#2a2d3a]">
                    ...
                    <div class="relative">
                        <div class="w-12 h-12 rounded-full border-2 border-[#2a2d3a] p-0.5">
                            <div class="w-full h-full rounded-full overflow-hidden bg-[#1a1d29]">
                                <img src="{{ $conversation->user->profile_photo ? asset('storage/' . $conversation->user->profile_photo) : asset('img/noprofile.jpg') }}" alt="{{ $conversation->user->first_name }}" class="w-full h-full object-cover">
                            </div>
                        </div>
                    </div>
        ...
        <main class="flex-1 flex flex-col bg-[#0f1117]">
            @if($selectedConversationId)
                @php
                    $activeChat = $conversations->where('id', $selectedConversationId)->first();
                @endphp
                <!-- Chat Header -->
                <div class="p-4 border-b border-[#2a2d3a] flex items-center justify-between bg-[#12151e]">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-full overflow-hidden border border-[#2a2d3a] bg-[#1a1d29]">
                            <img src="{{ $activeChat->user->profile_photo ? asset('storage/' . $activeChat->user->profile_photo) : asset('img/noprofile.jpg') }}" alt="{{ $activeChat->user->first_name }}" class="w-full h-full object-cover">
                        </div>
                        <div>
                            <h4 class="text-sm font-bold text-white">{{ $activeChat->user->first_name }} {{ $activeChat->user->last_name }}</h4>
                        ...
                        @else
                            ...
                            <div class="flex gap-3 max-w-[80%]">
                                <div class="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 bg-[#252836] border border-[#2a2d3a] overflow-hidden">
                                    <img src="{{ $activeChat->user->profile_photo ? asset('storage/' . $activeChat->user->profile_photo) : asset('img/noprofile.jpg') }}" alt="User" class="w-full h-full object-cover">
                                </div>
                                <div>
