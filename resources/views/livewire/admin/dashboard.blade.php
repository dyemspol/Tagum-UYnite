        ...
        <div class="">
            ...
            <div class="grid grid-cols-1 grid-row-2 sm:grid-cols-2 md:grid-cols-3 gap-6">
                ...
                <div class="bg-gradient-to-br from-[#1e2130] to-[#252836] rounded-xl p-5 flex flex-col justify-between shadow-md border border-[#2a2d3a]">
                    ...
                    <div class="mt-3">
                        <h2 class="text-3xl font-bold text-white">{{ $staff ?? 0 }}</h2>
                ...
                <div class="bg-gradient-to-br from-[#1e2130] to-[#252836] rounded-xl p-5 flex flex-col justify-between shadow-md border border-[#2a2d3a]">
                    ...
                    <div class="mt-3">
                        <h2 class="text-3xl font-bold text-white">{{ $staffdepartment ?? 0 }}</h2>
                ...
                <div class="bg-gradient-to-br from-[#1e2130] to-[#252836] rounded-xl p-5 flex flex-col justify-between shadow-md border border-[#2a2d3a]">
                    ...
                    <div class="mt-3">
                        <h2 class="text-3xl font-bold text-white">{{ $users ?? 0 }}</h2>
                        <p class="text-yellow-500 text-sm mt-1">Accounts waiting for review</p>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-6 bg-[#1a1d29] p-4 rounded-lg shadow border border-[#2a2d3a]">
                ...
                <div class="space-y-3 max-h-[250px] overflow-y-auto hide-scrollbar">
                    ...
                    <div class="flex justify-between items-center bg-[#252836] p-3 rounded">
                        <div>
                            <p class="text-white text-sm">{{ ucfirst($user->first_name) }} {{ ucfirst($user->last_name) }}</p>
                            <p class="text-xs text-gray-300">Registered · {{ $user->created_at->diffForHumans() }}</p>
                        </div>
                        <button wire:click="showUser({{ $user->id }})" class="text-white bg-[#2a2d3a] hover:bg-[#363944] px-2 py-1 rounded">View</button>
                    </div>
                    @endforeach
                </div>
            </div>
