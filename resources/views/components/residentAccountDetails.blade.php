...
<div
    ...
    @if($selectedUser)
    <!-- Modal Panel -->
    <div class="relative w-full max-w-md mx-4 bg-[#12151e] light:bg-[#f8fafc] rounded-2xl shadow-2xl border border-[#2a2d3a] light:border-gray-200 overflow-hidden transition-colors">
        ...
        <div class="bg-[#1a1d29] light:bg-white px-6 py-4 flex items-center border-b border-[#2a2d3a] light:border-gray-200 transition-colors">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-white light:text-gray-900 font-semibold text-sm transition-colors">Resident Details</h3>
        ...
        <div class="flex flex-col items-center py-5 border-b border-[#2a2d3a] light:border-gray-200 bg-[#12151e] light:bg-[#f8fafc] transition-colors">
            ...
            <p class="text-white light:text-gray-900 font-semibold text-sm mt-3 transition-colors">{{ ucfirst($selectedUser->first_name) }} {{ ucfirst($selectedUser->last_name) }}</p>
        ...
        <div class="px-6 py-4 grid grid-cols-1 gap-3">
            ...
            <div class="grid grid-cols-2 gap-3">
                <div class="flex items-start gap-3 bg-[#1a1d29]/80 light:bg-white rounded-xl px-4 py-3 transition-colors">
                    ...
                    <div>
                        ...
                        <p class="text-white light:text-gray-900 text-sm transition-colors">{{ ucfirst($selectedUser->first_name) }}</p>
                    </div>
                </div>
                ...
                <div class="flex items-start gap-3 bg-[#1a1d29]/80 light:bg-white rounded-xl px-4 py-3 transition-colors">
                    ...
                    <div>
                        ...
                        <p class="text-white light:text-gray-900 text-sm transition-colors">{{ ucfirst($selectedUser->last_name) }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-start gap-3 bg-[#1a1d29]/80 light:bg-white rounded-xl px-4 py-3 transition-colors">
                ...
                <div>
                    ...
                    <p class="text-white light:text-gray-900 text-sm truncate transition-colors">{{ $selectedUser->email }}</p>
                </div>
            </div>
            ...
            <div class="flex items-start gap-3 bg-[#1a1d29]/80 light:bg-white rounded-xl px-4 py-3 transition-colors">
                ...
                <div>
                    <p class="text-gray-400 light:text-gray-500 text-xs mb-0.5 transition-colors">Address</p>
                    <p class="text-white light:text-gray-900 text-sm transition-colors">{{ Str::title($selectedUser->address ?? 'N/A') }}, {{ Str::title($selectedUser->barangay->barangay_name ?? 'N/A') }}, Tagum City</p>
                </div>
            </div>
            ...
            <div class="flex items-start gap-3 bg-[#1a1d29]/80 light:bg-white rounded-xl px-4 py-3 transition-colors">
                ...
                <div>
                    <p class="text-gray-400 light:text-gray-500 text-xs mb-0.5 transition-colors">Resident ID</p>
                    <p class="text-white light:text-gray-900 text-sm font-mono transition-colors">#RES-{{ $selectedUser->id }}</p>
                </div>
            </div>
            ...
            <div class="flex items-start gap-3 bg-[#1a1d29]/80 light:bg-white rounded-xl px-4 py-3 transition-colors">
                ...
                <div>
                    ...
                    <p class="text-white light:text-gray-900 text-sm transition-colors">{{ $selectedUser->created_at->format('F j, Y') }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="px-6 pb-6">
            <button wire:click="closeUser"
                class="w-full py-2.5 rounded-xl bg-[#1a1d29] light:bg-gray-100 hover:bg-[#252836] light:hover:bg-gray-200 text-gray-300 light:text-gray-800 hover:text-white light:hover:text-gray-900 border border-[#2a2d3a] light:border-gray-300 text-sm font-medium transition-colors">
                Close
            </button>
        </div>
