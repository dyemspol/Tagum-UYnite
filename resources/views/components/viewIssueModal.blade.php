...
<div
    ...
    <!-- MODAL CONTAINER -->
    <div class="bg-[#1a1d29] light:bg-white w-full max-w-5xl rounded-3xl shadow-2xl flex overflow-hidden border border-[#2a2d3a] light:border-gray-200 relative transition-colors"
    ...
    new Swiper('.mySwiper', {
        ...
        <!-- CLOSE BUTTON -->
        <button wire:click="closeIssue" class="absolute top-6 right-6 w-10 h-10 flex items-center justify-center rounded-full bg-[#12151e] light:bg-white border border-[#2a2d3a] light:border-gray-200 text-gray-400 hover:text-white light:hover:text-gray-900 hover:bg-[#2a2d3a] light:hover:bg-gray-100 transition-all z-20">
        ...
        <div class="w-[55%] p-8 border-r border-[#2a2d3a] light:border-gray-200 overflow-y-auto hide-scrollbar max-h-[90vh] transition-colors">
            ...
            <div class="flex items-center justify-between mb-8">
                <div>
                    <h2 class="text-2xl font-bold text-white light:text-gray-900 tracking-tight">Issue Details</h2>
            ...
            <div class="grid grid-cols-2 gap-8 text-sm">
                <div class="col-span-2">
                    <p class="text-gray-500 light:text-gray-400 text-[10px] font-bold uppercase tracking-widest mb-1">Issue Name</p>
                    <p class="text-white light:text-gray-900 text-lg font-semibold">{{ $selectedReport->title }}</p>
                </div>

                <div>
                    <p class="text-gray-500 light:text-gray-400 text-[10px] font-bold uppercase tracking-widest mb-1">Reported by</p>
                    <div class="flex items-center gap-2">

                        <p class="text-gray-200 light:text-gray-800 font-medium">{{ $selectedReport->user->first_name }} {{ $selectedReport->user->last_name }}</p>
                    </div>
                </div>

                <div>
                    <p class="text-gray-500 light:text-gray-400 text-[10px] font-bold uppercase tracking-widest mb-1">Date Reported</p>
                    <p class="text-gray-200 light:text-gray-800 font-medium">{{ $selectedReport->created_at->format('M d, Y') }}</p>
                </div>

                <div class="col-span-2">
                    <p class="text-gray-500 light:text-gray-400 text-[10px] font-bold uppercase tracking-widest mb-1">Description</p>
                    <p class="text-gray-300 light:text-gray-700 leading-relaxed bg-[#12151e] light:bg-[#f3f4f6] p-4 rounded-xl border border-[#2a2d3a]/50 light:border-gray-200 transition-colors">
                        {{ $selectedReport->content }}
                    </p>
                </div>

                <div class="col-span-2">
                    <p class="text-gray-500 light:text-gray-400 text-[10px] font-bold uppercase tracking-widest mb-1">Location</p>
                    <div class="flex items-center gap-2 text-gray-200 light:text-gray-800">
            ...
            <!-- SWIPER CAROUSEL -->
            <div class="mt-8">
                <p class="text-gray-500 light:text-gray-400 text-[10px] font-bold uppercase tracking-widest mb-3">Attached Proof</p>
                <div class="swiper mySwiper h-80 rounded-2xl border border-[#2a2d3a] light:border-gray-200 transition-colors">
                    <div class="swiper-wrapper">
                        ...
                        <div class="swiper-slide group cursor-zoom-in" @click="fullscreenImage = '{{ $image->cdn_url }}'; showFullscreen = true">
                            <img src="{{ $image->cdn_url }}" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110 light:bg-gray-100" />
                        </div>
                        @endforeach
                    </div>
                ...
                <p class="text-center text-[10px] text-gray-500 light:text-gray-400 mt-2 italic">Click image to view full screen</p>
            </div>
        </div>

        <!-- RIGHT PANE: POST COMMENTS -->
        <div class="flex-1 bg-[#12151e]/50 light:bg-[#f8fafc] p-8 flex flex-col max-h-[90vh] transition-colors">
            <div class="flex items-center justify-between mb-8">
                <h3 class="text-lg font-bold text-white light:text-gray-900 flex items-center gap-2">
            ...
            <div class="flex-1 overflow-y-auto space-y-4 mb-8 hide-scrollbar pr-1">
                ...
                <div class="p-4 bg-[#1a1d29] light:bg-white rounded-2xl border border-[#2a2d3a] light:border-gray-200 shadow-sm transition-colors">
                    <div class="flex justify-between items-center mb-1">
                        <span class="text-xs font-bold text-white light:text-gray-900">{{ $comment->user->first_name }} {{ $comment->user->last_name }}</span>
                        <span class="text-[10px] text-gray-500 light:text-gray-400">{{ $comment->created_at->diffForHumans() }}</span>
                    </div>
                    <p class="text-[11px] text-gray-300 light:text-gray-700 leading-relaxed">{{ $comment->comment_text }}</p>
                </div>
                @empty
            ...
            <div class="mb-6">
                <div class="relative group">
                    <textarea
                        ...
                        class="w-full bg-[#1a1d29] light:bg-white border border-[#2a2d3a] light:border-gray-300 rounded-2xl p-4 pr-12 text-xs text-white light:text-gray-800 focus:outline-none focus:ring-1 focus:ring-[#00d4aa] min-h-[60px] transition-all resize-none shadow-inner"></textarea>
                    <button
            ...
            <!-- COMPACT CONTROLS -->
            <div class="space-y-4 bg-[#1a1d29] light:bg-[#f1f5f9] p-6 rounded-3xl border border-[#2a2d3a] light:border-gray-200 shadow-2xl transition-colors">
                <div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="space-y-3 pt-4 border-t border-[#2a2d3a]/50 light:border-gray-200">
                        <label class="text-[10px] text-gray-500 light:text-gray-600 font-bold uppercase tracking-widest">Takedown Action</label>
                        <select
